...
Used by :func:`routir.config.load.auto_add_relay_services` to register a local
...
:mod:`routir.collections.processor` does — today the relay forwards whole
...
class RelayContentProcessor(Processor):
    ...
    The processor mirrors :class:`~routir.collections.processor.ContentProcessor`
    ...
    def __init__(
        ...
        # Populated by ``auto_add_relay_services`` from the remote ``/avail``
        # so ``SearchPipeline.verify()`` can read views / default_view exactly
        # the way it reads ``ContentProcessor.backends`` for local collections.
